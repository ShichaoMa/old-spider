...
from toolkit import safely_json_loads, groupby, chain_all

from . import BaseItem


def product_id_processor(values, loader_context):
    if loader_context["item"]["initialData"]:
...
def features_processor(values, loader_context):
    if loader_context["item"]["initialData"]:
        return loader_context["item"]["initialData"]["Model"]["StyleModel"]["Features"]
...
class NordstromItem(BaseItem):
    initialData = Field(input_processor=MapCompose(safely_json_loads), default=dict(), skip=True)
