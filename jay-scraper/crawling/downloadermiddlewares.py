...
from toolkit import parse_cookie, get_ip, timeout
...
from .utils import CustomLogger, get_val
    ...
    def _retry(self, request, reason, spider):
        ...
        if request.meta.get("if_next_page"):
            ...
            if request.meta.get("callback") == "parse":
                spider.crawler.stats.inc_total_pages(crawlid=request.meta['crawlid'])
            self.logger.error(
                "in %s retry request error to failed pages url:%s, exception:%s, meta:%s" % (get_ip(),
                                                                                             request.url, reason,
                                                                                             request.meta))
        ...
        if request.meta.get("if_next_page"):
            ...
            self.logger.error(
                "In %s exception_retry request error to failed pages url:%s, exception:%s, meta:%s. " % (
                    get_ip(), request.url, reason, request.meta))
    ...
    def _retry(self, request, reason, spider):
        ...
        if request.meta.get("if_next_page"):
            ...
            if request.meta.get("callback") == "parse":
                spider.crawler.stats.inc_total_pages(crawlid=request.meta['crawlid'])
            self.logger.error(
                "in %s retry request error to failed pages url:%s, exception:%s, proxy:%s" % (get_ip(),
                                                                                             request.url, reason,
...
class JomashopCpatchaMiddleware(DownloaderBaseMiddleware):
    ...
    @process_requset_method_wrapper
    def process_request(self, request, spider):
        cookies = spider.redis_conn.hget("jomashop:cookies", "cookies") or ""
        if cookies:
            self.logger.debug("Add captcha cookies: %s"%cookies)
            request.headers["Cookie"] = cookies

    @process_response_method_wrapper
    def process_response(self, request, response, spider):
        if response.status in [200, 405] and len(response.body) < 5000:
            if response.status == 200:
                return request
            else:
                if float(spider.redis_conn.hget("jomashop:cookies", "refresh_timeline") or time.time()) < time.time():
                    spider.redis_conn.hmset("jomashop:cookies", {"expire": True, "refresh_timeline": time.time()})
...
class AmazonCurlCaptchaRedisCachedMiddleware(DownloaderBaseMiddleware):
    ...
    def get_captcha(self, img_url, proxy=None):
        try:
            ...
            if proxy:
                proxies = {
                    "https": 'http://%s@%s' % (
                        self.settings.get("PROXY_ACCOUNT_PASSWORD", ""), proxy.lstrip("https://").lstrip("http://"))}
            else:
                proxies = None
            resp = get(img_url, proxies=proxies, verify=False)
            img_body = b""
    ...
    def parse_captcha(self, img_body):
        try:
            ...
            client = socket.socket()
            client.connect(
                (self.settings.get("CAPTCHA_HOST", "192.168.200.58"), self.settings.get("CAPTCHA_PORT", 8887)))
            client.send(img_body)
            alphabet = client.recv(1024)
            client.close()
            return alphabet
