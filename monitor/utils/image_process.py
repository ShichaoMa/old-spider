...
from . import wrapper_self, InvalidateImageError


influxdb_client = InfluxDBClient('192.168.200.131', 8086, "", "", 'db_roc')
...
def duration_gather(name, data):
    def inner(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            t1 = time.time()
            result = func(*args, **kwargs)
            data[name] = time.time() - t1
            return result
        return wrapper
    return inner
...
class ImageProcess(ProxyPool):
    ...
    def process_image(self, spider, document):
        data = dict()
        data["worker_id"] = self.name
        data["total_pic_count"] = len(document.get("images"))
        data["local_pic_count"] = Queue()
        data["remote_pic_count"] = Queue()
        error_packages = duration_gather("download_duration", data)(self.process_download)(document, data)
        if spider == "loco_amazon":
            return True
        result = error_packages is not False and \
                 duration_gather("convert_duration", data)(
                     wrapper_self(self)(self.generate_convert(CONVERTIONS.get(spider, {}))))(document, error_packages)
        data["local_pic_count"] = data["local_pic_count"].qsize()
        data["remote_pic_count"] = data["remote_pic_count"].qsize()
        tags = {}
        print(data)
        with ExceptContext():
            influxdb_client.write_points([{
                "measurement": "image_cost",
                "tags": tags,
                "time": datetime.datetime.utcnow(),
                "fields": data
            }])
    ...
    def process_download(self, document, data):
        """
        ...
        with ExceptContext(errback=self.log_err) as ec:
            for image_package in document.get("images", []):
                if image_package:
                    self.download_queue.put((image_package, error_packages, data))
                    self.lock.acquire()
                    self.pic_count += 1
                    self.lock.release()
            while self.pic_count > 0:
                time.sleep(0.1)
        return not ec.got_err and error_packages
    ...
    def download(self):
        ...
        while True:
            try:
                image_package, error_packages, data = self.download_queue.get_nowait()
            except Empty:
                image_package = None
            if image_package:
                with ExceptContext(errback=self.log_err, finalback=_final):
                    ...
                    if session.head(filename).status_code < 400:
                        ...
                        data["local_pic_count"].put(1)
                        continue
                    data["remote_pic_count"].put(1)
