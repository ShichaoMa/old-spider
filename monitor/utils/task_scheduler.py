...
from .queues import FifoDiskQueue, RedisHashSet
...
class RedisTaskScheduler(object):
    ...
    def __init__(self, redis_url, default_delay, task_name, identity, path="."):
        self.identity = identity
        self.redis_conn = Redis.from_url(redis_url)
        self.input_channel = FifoDiskQueue(os.path.join(path, "input"))
        self.output_channel = FifoDiskQueue(os.path.join(path, "output"))
