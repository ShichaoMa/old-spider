...
from .utils import enrich_wrapper, CustomLoader, re_search, xpath_exchange
...
class FinishlineSpider(JaySpider):
    ...
    custom_settings = {
        "USER_AGENT": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3222.0 Safari/537.36",
        "ITEM_PIPELINES": {
    ...
    def enrich_data(self, item_loader, response):
        ...
        sizes = dict((xpath_exchange(size.xpath("@id")),
                      [{base64.decodebytes(xpath_exchange(option.xpath("@data-sku")).encode()):
                            xpath_exchange(option.xpath("@aria-label"))}
                       for option in size.xpath("ul/li/a")]) for size in response.css('div.sizeList'))
        item_loader.add_value("sizes", sizes)
        item_loader.add_value("colors", dict(zip(response.xpath(
            '//div[@id="alternateColors"]/div/a/@data-productid').extract(), response.xpath(
            '//div[@id="alternateColors"]/div/a/div/img/@alt').extract())))
        item_loader.add_xpath("image_urls", '//div[@id="mobileImageSlider"]/div/img/@src')
