...
from .utils import enrich_wrapper, re_search, CustomLoader, xpath_exchange,\
    safely_json_loads, ItemCollectorPath, rid, strip, get_from_other_site
...
class AmazonSpider(JaySpider):
    ...
    custom_settings = {
        "ITEM_PIPELINES": {
            ...
            # 'crawling.pipelines.FilePipeline': 101,
        },
        ...
        "SPIDER_MIDDLEWARES": {
            ...
            'crawling.spidermiddlewares.AmazonMiddleware': 991,
        }
    }
        ...
        if next_url:
            ...
            if not next_url.count("redirect") or next_url.count("redirect") and offer:
                ...
                try:
                    ...
                    self.logger.info("Error in offer listing request: %s" % e)
