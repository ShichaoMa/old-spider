...
from ..items.eastbay_item import EastbayItem, image_urls_processor, color_images_processor
from .utils import enrich_wrapper, re_search, CustomLoader, safely_json_loads, re_exchange, ItemCollectorPath
    ...
    def enrich_image_urls(self, item_loader, response):
        ...
        item_loader.add_value("color_images", (
            re_search(r"(?<=EBFL2/)(.*?)(?=\?)", response.url), image_urls_processor(safely_json_loads(
                re_exchange(response.selector.re(r'\((.*?),""\)'))))), color_images_processor)
