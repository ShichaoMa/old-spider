...
from ..utils import re_search, groupby, format_html_string,\
    safely_json_loads, TakeAll, replace_dot, wrap_key, chain_all
...
class BackcountryItem(BaseItem):
    ...
    title = Field()
    description1 = Field(input_processor=MapCompose(format_html_string))
    description2 = Field(input_processor=MapCompose(format_html_string))
